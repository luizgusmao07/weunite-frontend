...
export default function HeaderProfile() {
  ...
  return (
    <>
      ...
      <div>
        <div
          className={`h-28 sm:h-32 md:h-35 lg:h-44 xl:h-40 relative ${isDesktop ? "flex justify-center" : ""}`}
        >
          <img
            className={`h-full w-full object-cover rounded-b-sm ${isDesktop ? "max-w-2xl" : ""}`}
        ...
        <div
          className={`${isDesktop ? "flex flex-col px-115" : `flex w-full ${isTablet ? "px-4" : ""}`}`}
        >
          <div
            className={`${isDesktop ? "flex items-end justify-between mt-[-50px]" : "flex items-end gap-3 ml-2 mt-[-50px]"}`}
          >
            <div className="flex items-end gap-8">
              ...
              {/* info usuário */}
              <div
                className={`${isDesktop ? "flex flex-col mb-2" : "flex flex-col mb-2"}`}
              >
                <p
                  className={`text-primary font-medium ${isDesktop ? "text-2xl" : "text-base"}`}
                >
                  {user?.username}
                </p>
